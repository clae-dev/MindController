...
청소년 행사·부스 체험용 **웹 기반 표정 스트레스 분석 서비스**입니다. 웹캠 앞에서 10초면 표정과 심박을 읽어 스트레스 지수와 감정 상태, 그리고 마음에 닿는 한마디를 전해줍니다.
...
- **심박수 추정 (rPPG)** — 손 대지 않고 웹캠만으로 이마 피부의 미세한 맥동을 분석해 BPM 추정 (참고·재미용)
...
3. 10초 평균 → 스트레스 지수 = 슬픔+분노+불쾌+불안×0.8+놀람×0.3−행복×0.5. 이마 ROI 색 변화로 심박(rPPG) 별도 추정
